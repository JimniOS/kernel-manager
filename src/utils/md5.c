...
static void md5Update(MD5Context *ctx, uint8_t *input, size_t input_len);
...
static uint8_t *md5File(FILE *file);
...
static void 
md5Update(MD5Context *ctx, uint8_t *input_buffer, size_t input_len)
{
        uint32_t input[16];
...
// declared in header
// wrapper for internal MD5 operations
void 
md5sum(FILE *file_src, FILE *dest)
{
        uint8_t *result_src = md5File(file_src);
        uint8_t *result_dest = md5File(dest);

        long diff = 0;
        for (int i = 0; i < 16; i++)
        {
                diff += result_src[i] - result_dest[i];
        }
        if (diff != 0)
        {
